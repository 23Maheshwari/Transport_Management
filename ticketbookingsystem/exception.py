...
class TicketBookingSystem:
    ...
    def book_tickets(self, event_name: str, num_tickets: int, customers: List[str]):
        ...
        for event in self.events:
            if event.name == event_name:
                ...
                break
        if not event_found:
            raise EventNotFoundException("Event not found for booking.")
    ...
    def cancel_tickets(self, booking_id: int):
        ...
        if not booking_found:
            raise InvalidBookingIDException("Invalid booking ID for cancellation.")

    def get_available_seats(self, event_name: str):
        for event in self.events:
            if event.name == event_name:
                return event.get_available_no_of_tickets()
        raise EventNotFoundException("Event not found for available seats.")

    def get_event_details(self, event_name: str):
        for event in self.events:
            if event.name == event_name:
                return event.get_event_details()
        raise EventNotFoundException("Event not found for details.")
...
class Event:
    ...
    def cancel_booking(self, booking_id: int):
        if self.booked_seats > 0 and self.booking_id == booking_id:
            self.booked_seats -= 1
